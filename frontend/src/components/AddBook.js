...
function FormDisabledInputExample() {
  ...
  const handleSubmit = event => {
    ...
    axios.post('http://localhost:5000/AddBook',values)
    .then(res => {
      if (res.data.Admin) {
        
        navigate('/Admin');
        alert('Succeful,the book is added');
      } else {
  ...
  return (
    ...
    <Row className="  d-flex justify-content-center align-items-center">
            ...
            <Card  className="shadow mt-5 ml-5" style={{ backgroundPosition:'center center ',backgroundSize:'cover',backgroundColor:'rgba(106, 132, 34, 0.22)',color:'rgb(255, 255, 255)'}} >
              <Card.Body style={{fontStyle: 'italic',fontsize: 'x-large'}}>
                <div className="mb-3 mt-md-4">
                  ...
                  <p className=" mb-2">Add Book!</p>
                  <div className="mb-3">
                  <Form  onSubmit={handleSubmit}>
      <Form.Group  >
        <Form.Label style={{fontStyle: 'italic',fontsize: 'x-large'}}>Title</Form.Label>
        <Form.Control  className='mb-4 w-100' type="text" name="Title" onChange={handleChange} value={values.Title} required/>
      ...
      <Form.Group  controlId="formBasicPassword">
        ...
        <Form.Control className='mb-4 w-100' type="text" name="Author" onChange={handleChange} value={values.Author} required />
      </Form.Group>

      <Form.Group  controlId="formBasicPassword">
        <Form.Label style={{fontStyle: 'italic',fontsize: 'x-large'}}>ISBN</Form.Label>
        <Form.Control className='mb-4 w-100' type="text" name="ISBN" onChange={handleChange} value={values.ISBN} required/>
      </Form.Group>
      <Form.Group  controlId="formBasicPassword">
        <Form.Label style={{fontStyle: 'italic',fontsize: 'x-large'}}>Summary</Form.Label>
        <Form.Control className='mb-4 w-100' type="text" name="Summary" onChange={handleChange} value={values.Summary} required/>
      </Form.Group>
      <Form.Group controlId="formBasicPassword">
     ...
     <Form.Group controlId="formFile" className='padd'>
        <Form.Label style={{fontStyle: 'italic',fontsize: 'x-large'}}>image</Form.Label>
        <Form.Control  className='mb-4 w-100' type="file" name="photo" onChange={handleChange} value={values.photo} required/>
      </Form.Group>
